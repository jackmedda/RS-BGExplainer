...
class Explainer:
    ...
    def __init__(self, config, dataset, rec_data, model, dist="damerau_levenshtein", **kwargs):
        ...
        self.users_low_degree_ratio = config['users_low_degree_ratio'] or 0.2
...
#         )
    ...
    def _check_policies(self, batched_data, rec_model_topk):
        ...
        if self.users_low_degree and self.random_perturbation:
            ...
            lowest_degree = torch.argsort(hist_len)[:int(self.users_low_degree_ratio * hist_len.shape[0])]
    ...
    def explain(self, batched_data, full_dataset, train_data, valid_data, test_data, epochs, topk=10):
        ...
        full_users_list = np.arange(1, full_dataset.user_num)
        ...
        full_pref_data = self._pref_data_sens_and_metric(
            full_users_list, full_users_test_model_topk, eval_data=test_data.dataset
        )
        self.logger.info(f"Original Test Overall {self.eval_metric.upper()}: {full_pref_data[self.eval_metric].mean()}")
